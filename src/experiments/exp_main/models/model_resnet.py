...
class ResNet(torch.nn.Module):
    ...
    def __init__(self):
        ...
        # 5th stage
        self._res_conv_bl5 = ResConv(in_channels=1024, f1=512, f2=2048, stride=1)
        self._res_identity_bl5 = ResIdentity(512, 2048)

        # 6th stage
        self.pool_avg = torch.nn.AvgPool2d(kernel_size=2)

        self.fc1 = torch.nn.Linear((2048 * 3 * 3), 1024)
        self.fc_act1 = torch.nn.ReLU()

        self.fc2 = torch.nn.Linear(1024, 512)
        self.fc_act2 = torch.nn.ReLU()
        self.fc_drop2 = torch.nn.Dropout(p=0.5)
    ...
    def forward(self, bands):
        ...
        # 4th stage
        bands = self._res_conv_bl4(bands)
        bands = self._res_identity_bl4(bands)
        bands = self._res_identity_bl4(bands)
        bands = self._res_identity_bl4(bands)
        bands = self._res_identity_bl4(bands)
        bands = self._res_identity_bl4(bands)
        ...
        # 5th stage
        bands = self._res_conv_bl5(bands)
        bands = self._res_identity_bl5(bands)
        bands = self._res_identity_bl5(bands)
