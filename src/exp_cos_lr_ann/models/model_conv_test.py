...
class ConvNet(torch.nn.Module):
    ...
    def __init__(self):
        ...
        # pre_fc layers
        self.pre_fc1 = torch.nn.Linear(13 * 64 * 64, 4096)
        self.pre_fc1_act = torch.nn.ReLU()

        self.pre_fc2 = torch.nn.Linear(4096, 1024)
        self.pre_fc2_act = torch.nn.ReLU()

        self.pre_fc3 = torch.nn.Linear(1024, 13 * 32 * 32)
    ...
    def forward(self, bands):
        ...
        bands = self.pre_fc1(bands)
        bands = self.pre_fc1_act(bands)

        bands = self.pre_fc2(bands)
        bands = self.pre_fc2_act(bands)

        bands = self.pre_fc3(bands)

        bands = bands.view(bands.size(0), 13, 32, 32)
